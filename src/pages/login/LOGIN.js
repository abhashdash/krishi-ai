...
function validateMobile(mobile) {
    return (window.KrishiValidate && window.KrishiValidate.isMobile(mobile)) || (/^[6-9]\d{9}$/.test(mobile));
}

function validateOTP(otp) {
    return (window.KrishiValidate && window.KrishiValidate.isOTP(otp)) || (/^\d{6}$/.test(otp));
}
...
loginForm.addEventListener('submit', function(e) {
    ...
    // Attempt login via KrishiAuth if available, otherwise simulate
    setTimeout(() => {
        if (window.KrishiAuth) {
            const res = window.KrishiAuth.loginWithOTP(mobile, otp);
            if (res.success) {
                showNotification('Login successful! Redirecting...', 'success');
                setTimeout(() => { window.location.href = '../home/HOME.html'; }, 900);
                return;
            }
            showError('otp', res.message || 'Invalid OTP. Please try again.');
